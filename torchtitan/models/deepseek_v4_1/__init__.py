...
from .indexer import FULL, HierarchicalIndexer, IndexerDistillLoss, REINDEX, REUSE
...
def _linear(in_features: int, out_features: int) -> Linear.Config:
    """Bias-free linear with the model's default init."""
...
def _rmsnorm(normalized_shape: int, eps: float) -> RMSNorm.Config:
    return RMSNorm.Config(
        normalized_shape=normalized_shape, eps=eps, param_init=_NORM_INIT
    )
...
def _make_compressor_config(
    *,
    ...
    return Compressor.Config(
        ...
        is_source=is_source,
        rope=rope,
        wkv=_linear(dim, head_dim) if is_source else None,
        # The softmax gate only exists when there is more than one token to pool.
        wgate=_linear(dim, head_dim) if is_source and compress_ratio > 1 else None,
        norm=_rmsnorm(head_dim, norm_eps) if is_source else None,
    )
...
def _make_indexer_config(
    *,
    ...
    return HierarchicalIndexer.Config(
        ...
        rope=rope,
        wq_b=(
            _linear(q_lora_rank, index_n_heads * index_head_dim) if is_source else None
        ),
        weights_proj=_linear(dim, index_n_heads) if is_source else None,
        wk=_linear(head_dim, index_head_dim) if owns_index_k else None,
        k_norm=_rmsnorm(index_head_dim, norm_eps) if owns_index_k else None,
    )
...
def _make_attention_config(
    *,
    ...
    # predict a selection that is jointly useful for the whole group. Because dI is
    # affine in the teacher, the per-layer losses sum (with no group-size divisor) to the
    # gradient of the pooled teacher. The student logits at the selected entries are the
    # expensive half of the indexer forward (a gather plus an einsum with a graph), so
    # they are only produced where a loss will consume them and only while training.
    aux_loss = None
    if indexer_loss_coeff is not None and compresses:
        if any(s <= layer_id for s in index_source_layers):
    ...
    return Attention.Config(
        ...
        head_dim=head_dim,
        compress_ratio=compress_ratio,
        inner_attention=CompressedSparseInnerAttention2.Config(
        ...
        q_norm=_rmsnorm(q_lora_rank, norm_eps),
        wq_b=_linear(q_lora_rank, n_heads * head_dim),
        wkv=_linear(dim, head_dim),
        kv_norm=_rmsnorm(head_dim, norm_eps),
        ...
        wo_b=_linear(n_groups * o_lora_rank, dim),
...
def _build_layers(
    *,
    ...
    # Every block carries the same MoE stack, so one config serves them all. ``build()``
    # copies a config before constructing its owner, so sharing it is not aliasing.
    moe_cfg = make_moe_config(
        ...
        shared_experts=(
            make_ffn_config(
                ...
                # The report clamps every expert's SwiGLU, the shared one
                # included; the routed experts carry the same limit.
                swiglu_limit=swiglu_limit,
            )
    ...
    for layer_id in range(n_layers):
        layers.append(
            DeepSeekV41TransformerBlock.Config(
                ...
                attention_norm=_rmsnorm(dim, norm_eps),
                ffn_norm=_rmsnorm(dim, norm_eps),
                moe=moe_cfg,
