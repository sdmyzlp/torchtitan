...
def _make_attention_config(
    *,
    ...
    # predict a selection that is jointly useful for the whole group. The student
    # logits at the selected entries are the expensive half of the indexer forward
    # (a gather plus an einsum with a graph), so they are only produced where a loss
    # will consume them and only while training.
    aux_loss = None
    ...
    needs_selection_scores = is_index_source and aux_loss is not None
