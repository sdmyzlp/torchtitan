...
class DeepSeekV41Metadata:
    ...
    ``doc_ids`` is the only field: a non-decreasing document index per token, built from
    the positions resetting to 0 at every packed segment start. The same tensor serves
    both consumers, which is why they agree by construction: ``selected_attention``
    applies ``doc_ids[q] == doc_ids[k]`` to its sliding-window branch, and the indexer
    applies the same equality one axis over, entry ``j`` covering tokens
    ``[j * compress_ratio, (j + 1) * compress_ratio)``. No ``cu_seqlens``-style ragged
    form is carried because the operators consume the per-token view directly.
    """
...
class DeepSeekV41Model(Decoder):
    ...
    def get_attention_masks(
        ...
        *,
        ...
        return DeepSeekV41Metadata(doc_ids_T=doc_ids_T)
