...
from .indexer import indexer_selection_masks
from .mhc import HcPost, HcPre


@dataclass(frozen=True, kw_only=True, slots=True)
class DeepSeekV41Metadata:
    """Per-forward varlen metadata, built by :meth:`DeepSeekV41Model.get_attention_masks`.

    ``doc_ids`` is the only per-token field: a non-decreasing document index per token,
    ...
    branch, and the indexer applies the same equality one axis over, entry ``j`` covering
    tokens ``[j * compress_ratio, (j + 1) * compress_ratio)``. No ``cu_seqlens``-style
    ragged form is carried because the operators consume the per-token view directly.
    ...
    doc_ids_T: torch.Tensor  # noqa: N815
...
class DeepSeekV41Model(Decoder):
    ...
    def get_attention_masks(
        ...
        *,
        ...
        ``selected_attention`` consumes the document ids for its window branch, and the
        indexer derives its entry-axis ``doc_ids[::compress_ratio]`` rule from them, so no
        ragged ``cu_seqlens`` form is needed. That rule is the same for every layer with
        the same pooling ratio, so it is evaluated here once per ratio rather than inside
        each indexer.
        """
        del padding_mask, max_num_documents, max_context_length
        doc_ids_T = torch.cumsum((positions == 0).to(torch.int32), dim=0) - 1
        return DeepSeekV41Metadata(
            doc_ids_T=doc_ids_T,
            selection_masks=indexer_selection_masks(doc_ids_T, self.compress_ratios),
        )
    ...
    def preprocess_inputs(
        ...
        *,
        ...
        """Attach the metadata before the shared decoder preprocessing runs.

        ``Decoder.preprocess_inputs`` only calls ``get_attention_masks`` for FlexAttention
        and VarlenAttention inner backends, so a model with a custom sparse core has to
        build its own mask and hand it to the base as an ordinary batch entry.
        """
        input_dict = dict(input_dict)
        positions = input_dict.get("positions")
        if positions is not None and input_dict.get("attention_masks") is None:
