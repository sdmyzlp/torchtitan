    ...
    for part in model_parts:
        for module in part.modules():
            if isinstance(module, LoggedAuxLoss):
                ...
                if key not in LoggedAuxLoss.group_acc:
                    LoggedAuxLoss.group_acc[key] = torch.zeros_like(module.instance_acc)
                LoggedAuxLoss.group_acc[key] += module.instance_acc
...
def collect_aux_loss_metrics(parallel_dims: ParallelDims) -> dict[str, float]:
    ...
    def _group_acc_or_zero(key: tuple[str, str]) -> torch.Tensor:
        group_acc_value = LoggedAuxLoss.group_acc.get(key)
    ...
    group_accs = {key: _group_acc_or_zero(key) for key in LoggedAuxLoss._group_counts}
    metrics = {}
