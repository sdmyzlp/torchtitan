...
class Indexer(Module):
    ...
    method, called inside the inner attention local_map region where
    ``idx_k`` has been CP-all-gathered to the full KV sequence.
    """
...
class DSAFlexAttention(FlexAttention):
    ...
    Inherits ``FlexAttention`` to trigger torchtitan's document-packing mask
    pipeline.  The first three positional args are ``q, k, v`` (CP contract);
    ``idx_q, idx_k, idx_w`` follow.  Under ``full_dtensor`` + CP the
    ``ShardingConfig`` gathers ``k/v/idx_k`` on the CP axis before this runs.
    """
    ...
    def build_dsa_block_mask(selected: torch.Tensor, bm: BlockMask) -> BlockMask:
        """Compose torchtitan's CP-sharded causal+document ``bm`` with DSA token
        selection into a combined BlockMask with block-skip sparsity.

        ``bm.mask_mod`` stays intact (handles document packing, CP remapping);
