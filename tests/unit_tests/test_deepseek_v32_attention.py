...
def _apply_eager_flex_patches():
    ...
    import torchtitan.models.deepseek_v3.model as _dsmodel
...
def _build_titan(index_topk):
    from torchtitan.models.deepseek_v3.__init__ import _make_dsv3_attn_config
    from torchtitan.models.common import ComplexRoPE

    rope = ComplexRoPE.Config(
        dim=QK_ROPE, max_seq_len=16384, theta=10000.0,
        scaling="yarn", rope_factor=40.0,
        beta_fast=32.0, beta_slow=1.0, original_seq_len=4096,
    )
    cfg = _make_dsv3_attn_config(
        layer_id=0,
        dim=DIM,
        n_heads=N_HEADS,
        q_lora_rank=Q_LORA,
...
def test_sparse_dsa_parity():
    ...
    from torchtitan.models.deepseek_v3.model import Indexer as TIndexer
    ...
    with torch.no_grad():
        ...
        topk_t, _ = TIndexer.select(iq, ik, iw, bm, topk)
        valid_t = (topk_t != -1).long()
        counts_t = torch.zeros(B, L, L, dtype=torch.long, device=topk_t.device)
        counts_t.scatter_add_(-1, topk_t * valid_t, valid_t)
        sel_t = (counts_t > 0) & causal_valid

        qr_ref = ref.q_norm(ref.wq_a(x))
        topk_ref = ref.indexer(x, qr_ref, 0, fc, causal2d)
        sel_ref = torch.zeros(B, L, L, dtype=torch.bool, device=topk_ref.device)
        sel_ref.scatter_(-1, topk_ref, True)
